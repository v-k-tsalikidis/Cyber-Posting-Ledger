"""
Publication-Quality HTML Report Exporter for Cyber Vacancy Intelligence Tracker.
Generates standalone, styled HTML assessment briefs with recruiter intelligence.
"""
...
def generate_html_report(vacancy: VacancyRecord, profile: CandidateProfile) -> str:
    ...
    status_color = "#0F766E" if fit_res.overall_status == "Eligible" else "#92400E" if fit_res.overall_status == "Conditional" else "#991B1B"
    status_bg = "#CCFBF1" if fit_res.overall_status == "Eligible" else "#FEF3C7" if fit_res.overall_status == "Conditional" else "#FFE4E6"

    certs_html = ""
    if rec:
        certs_html = "".join([
            f"<li style='margin-bottom:6px;'>{'<span style=\"color:#0F766E;font-weight:700;\">[HELD ✓]</span>' if c.held else '<span style=\"color:#92400E;font-weight:700;\">[TARGET 🎯]</span>'} <strong>{c.name} ({c.code})</strong> &bull; <em>{c.priority}</em></li>"
...
<body>
  <div class="report-card">
    ...
    <div class="footer">
      Generated by Vacancy Intelligence Ledger &bull; {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} &bull; Personal Confidential Career Evidence
    </div>
  </div>
</body>
</html>
